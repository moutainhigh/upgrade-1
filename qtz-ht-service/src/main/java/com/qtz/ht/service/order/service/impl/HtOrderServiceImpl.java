...
import com.qtz.ht.spi.enums.CommodityStatusEnum;
...
/**
 * <p>
 * Title:HtOrderServiceImpl
 * </p>
 * <p>
 * Description:商户订单表服务实现类
 * </p>
 * <p>
...
{
    /** 初始化日志对象 */
    private static Logger log = Logger.getLogger(HtOrderServiceImpl.class);
    ...
    @Override
    public List<OrderModel> calculateOrder(Map<Long, Integer> goodS) throws ServiceException
    {
    ...
    {
    	log.info("HtOrderServiceImpl buildOrder|params:goods"+goodS);
        ...
        for (HtBusiness b : listB)
        {// 按商家组装订单
            double orderPrice = 0d;
            double orderSumfinalPrice = 0d;
            List<HtStaffGoods> list2 = map.get(b.getDmId());
            for (HtStaffGoods g : list2)
            {// 计算支付价钱
                orderPrice = ArithUtil.add(orderPrice, ArithUtil.mul(g.getPrice(), goodS.get(g.getDmId())));
                // 计算订单的总成本价格
                orderSumfinalPrice = ArithUtil.add(orderSumfinalPrice,
                    ArithUtil.mul(g.getFinalPrice() == null ? 0d : g.getFinalPrice(), goodS.get(g.getDmId())));
        ...
        log.info("HtOrderServiceImpl buildOrder|params:list"+list);
        return list;
    }
	...
	public synchronized void addStock(List<OrderModel> list) throws ServiceException {
		...
		for (OrderModel o : list) {
			...
			if(listHtPlatformWalletLog!=null &&listHtPlatformWalletLog.size()>0){
				...
				platformRevenue = ArithUtil.add(platformRevenue, oldPwDB.getAmount()); //累加平台金额
				try {
	...
	public void addReconciliation(List<OrderModel> list) throws ServiceException {
		...
		for (OrderModel o : list) {
			...
			//平台折扣为1-9之间
			staffRevenue = ArithUtil.div(ArithUtil.mul(o.getPaymentPrice(), b.getSettDiscount()), 10, 2);
			//对账时间周期
			Integer  recoCycle = b.getRecoCycle();
			if (null == recoCycle) {
				recoCycle = Integer.valueOf(CfgHelper.getValue("FREEZING_TIME"));
			}
			//添加货款流水
			HtStaffPaymentFlow pf = new HtStaffPaymentFlow(b.getDmId(),o.getDmId(),o.getPaymentPrice(),
					ArithUtil.sub(o.getPaymentPrice().doubleValue(), staffRevenue),staffRevenue,
