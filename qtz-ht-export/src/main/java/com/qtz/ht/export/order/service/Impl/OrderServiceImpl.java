package com.qtz.ht.export.order.service.Impl;
...
import com.qtz.ht.export.order.dao.HtBusinessDao;
import com.qtz.ht.export.order.dao.OrderDao;
import com.qtz.ht.export.order.dao.UserAuthenDao;
import com.qtz.ht.export.order.entity.HtBusiness;
import com.qtz.ht.export.order.entity.HtPlatformWalletLog;
import com.qtz.ht.export.order.entity.HtStaffPaymentFlow;
import com.qtz.ht.export.order.entity.OrderLog;
import com.qtz.ht.export.order.entity.UserAuthen;
import com.qtz.ht.export.order.model.BulkShippingModel;
import com.qtz.ht.export.order.model.HtOrderResponseModel;
import com.qtz.ht.export.order.model.ModOrderModel;
import com.qtz.ht.export.order.model.OrderModel;
import com.qtz.ht.export.order.service.OrderService;
import com.qtz.ht.export.util.HtOrderEnum;
...
public class OrderServiceImpl implements OrderService  {
	...
	public int modBulkShipping(Long busiId, List<BulkShippingModel> list)throws ServiceException {
				...
				for (BulkShippingModel b : bufferList) {
					for (OrderModel s : subList) {
						if (b.getOrderId().longValue() == s.getDmId().longValue()) {
							pendingList.add(b);
							//设置日志
							OrderLog log = new OrderLog(idService.generateId(), s.getDmId(), time, HtOrderEnum.OrderStatus.waitTake.getId(), "商家发货");
							logList.add(log);
							
							double sellMoney =ArithUtil.div(ArithUtil.mul(s.getOrderPrice(), 9), 10, 2) ;
							business = htBusinessDao.getEntityById(s.getSellerId());
							if (null != business) {
								//平台折扣为1-9之间
								staffRevenue = ArithUtil.div(ArithUtil.mul(sellMoney, business.getSettDiscount()), 10, 2);
								//对账时间周期
								...
								HtStaffPaymentFlow supp = new HtStaffPaymentFlow(business.getDmId(),s.getDmId(),sellMoney,
										ArithUtil.sub(sellMoney, staffRevenue),staffRevenue,
										s.getPaymentMethod(),business.getSettDiscount(),time,
										time + 24 * 60 * 60 * 1000l * recoCycle,
										Constants.ONE,System.currentTimeMillis(),idService.generateId());
								suppList.add(supp);
								...
								HtPlatformWalletLog plat = new HtPlatformWalletLog(business.getDmId(),s.getDmId(),supp.getPlatformRevenue(),
										...
										supp.getReleaseTime(), Constants.ONE, supp.getCrtime(),idService.generateId());
								platList.add(plat);
								platformRevenue = ArithUtil.add(platformRevenue, plat.getAmount()); //累加平台金额
							}
						}
					}
				}
		...
		return result.length;
			...
			log.error(busiId + " "  + list.toArray() + e);
