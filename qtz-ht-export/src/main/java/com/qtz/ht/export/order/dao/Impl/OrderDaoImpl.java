...
public class OrderDaoImpl implements OrderDao{
	protected static Logger log = Logger.getLogger(OrderDaoImpl.class);
	 ...
	 public List<HtOrderResponseModel> findHtOrderResponseModels(Long busiId, Integer status, String detailAddr,
		...
		log.debug(sbSql);
		log.debug("params:{busiId="+busiId+",status="+status+",dmId="+dmId+",consignee="+consignee+
				",mphoneNo="+mphoneNo+",startCrtime="+startCrtime+",endCrtime="+endCrtime+",detailAddr="+detailAddr+"}");
		List<HtOrderResponseModel> dataset = orderJdbcTemplate.query(sbSql.toString(), params.toArray(), ParameterizedBeanPropertyRowMapper.newInstance(HtOrderResponseModel.class));
		return dataset;
	}
